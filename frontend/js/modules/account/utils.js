...
async function saveEdit(section) {
  ...
  const saveButton = btns?.querySelector(".save-btn");

  if (section === "dados") {
    const payload = {
      nome: document.getElementById("inp-nome")?.value.trim(),

      sobrenome: document.getElementById("inp-sobrenome")?.value.trim(),

      email: document.getElementById("inp-email")?.value.trim(),

      cpf: document.getElementById("inp-cpf")?.value,

      telefone: document.getElementById("inp-tel")?.value,

      data_nascimento: document.getElementById("inp-nasc")?.value || null,
    };

    console.log("[PERFIL] Dados preparados:", payload);

    /*
     * A chamada ao backend será adicionada
     * depois que confirmarmos a rota existente.
     */
  }
  ...
  if (saveButton) {
    ...
    saveButton.textContent = "Dados preparados";
    ...
    if (remaining === 0) {
      document.getElementById("favGrid").innerHTML = `
            <div class="empty-state" style="grid-column:1/-1">
                ...
                <p class="empty-sub">Salve peças que você ama para encontrá-las facilmente.</p>
...
function dismissNotif(id) {
  ...
  item.style.transition = "opacity .2s, max-height .3s .1s, padding .3s .1s";
