...
export async function carregarProduto() {
  try {
    ...
    document.getElementById("f-price").textContent = formatar(
      produto.preco_atual,
    );
    ...
    const sizesHtml = produto.tamanhos
      .map((t) => {
    ...
    // ✅ depois — formata os -> como bullets
    function formatarDescricao(texto) {
      ...
      return texto
        ...
        .map((t) => t.trim())
        .filter((t) => t.length > 0)
        .map((t, i) => (i === 0 ? `<p>${t}</p>` : `<p>• ${t}</p>`))
        .join("");
    }

    document.getElementById("f-desc").innerHTML = formatarDescricao(
