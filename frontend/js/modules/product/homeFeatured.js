...
import { notify } from "../../utils/notify.js?v=2";
...
function availableSizes(product) {
  return (product.tamanhos || []).filter(
    (size) => Number(size.estoque || 0) > 0,
  );
}
...
grid?.addEventListener("click", async (event) => {
  ...
  if (sizes.length !== 1) {
    window.location.href = appPath(
      `/produto?id=${encodeURIComponent(product.uuid)}`,
    );
    return;
  }
