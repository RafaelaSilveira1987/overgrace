import { carregarClientes } from "../../modules/client/listaAdmin.js";
...
document.getElementById("formClient").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    // 🔹 tamanhos
    const tamanhos = [
      ...document.querySelectorAll("#reporSizesContainer .repor-row"),
    ].map((row) => {
    ...
    let principal = document.querySelector(
      'input[name="imagem_principal"]:checked',
    );
    ...
    if (imagens.length > 1 && !principal) {
      throw new Error("Selecione a imagem principal");
    }
    ...
    Object.entries(dados).forEach(([key, value]) => {
      if (value === null || value === undefined) return; // ← pula nulos
    ...
    document
      .querySelectorAll("#sizeSelector .size-opt")
...
export async function editarProduto(id) {
  try {
    ...
    document.getElementById("f-date-start").value = dataUtil(
      produto.inicio_exibicao,
      "format",
      "Y-m-d",
    );
    document.getElementById("f-date-end").value =
      dataUtil(produto.fim_exibicao, "format", "Y-m-d") || "";
    ...
    setTimeout(() => {
      const radios = document.querySelectorAll(
        'input[name="imagem_principal"]',
      );

      if (radios[principalIndex]) {
        radios[principalIndex].checked = true;

        radios[principalIndex].closest(".img-item").classList.add("principal");
      }
    }, 0);
    ...
    produto.tamanhos.forEach((t) => {
      const el = [...document.querySelectorAll("#sizeSelector .size-opt")].find(
        (e) => e.textContent.trim() === t.tamanho,
      );
...
export async function removerProduto(id) {
  ...
  try {
    ...
    await alertConfirm("Produto excluído com sucesso!", "success");

    // atualizar grid
    carregarProdutos();
  } catch (e) {
    await alertConfirm("Erro ao excluir produto.", "error");
  }
}
